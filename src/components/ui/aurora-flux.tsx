...
void main() {
  ...
  vec3 glow = vec3(
    0.2 + 0.25 * sin(channel + 2.0),
    0.8 + 0.35 * sin(channel + 0.2),
    0.9 + 0.3 * sin(channel + 1.2)
  );
  O.rgb = vec3(O.b * 0.5, O.g * 1.25 + O.b * 0.3, O.r * 1.05) * glow;
...
export default function AuroraFlux({
  ...
  useEffect(() => {
    ...
    const loop = (t: number) => {
      ...
      gl.useProgram(program);
      if (uTimeRef.current) gl.uniform1f(uTimeRef.current, t * 0.00035);
